...
| **F1.13** | Encabezado, pie y layout base de la tienda | M | DR §5.1, §7 | Encabezado fijo con sombra al scrollear; pie con la zona de entrega y el retiro (RN-10), medios de pago y legales |
...
| ID | Tarea | Tam. | Referencias | Hecho cuando |
|---|---|---|---|---|
| **F2.1** | ABM de marcas, categorías y colores, con el **logo de marca** | M | FS RF-18 · TS §5.4, §9.1, §9.4 | Alta, edición y baja. No se puede borrar un elemento en uso: se ofrece desactivarlo (RN-11). La marca acepta un logo opcional: subirlo, reemplazarlo —borrando el anterior— y quitarlo. Borrar la marca se lleva sus archivos |
...
> **El lineamiento visual de F3.8 rige de acá en adelante.** El canvas aprobado en Claude Design (`DESIGN-REFERENCE.md` §1.3) cubre siete pantallas —home, catálogo, categorías, ficha, carrito, orden enviada y panel— y F3.8 lo bajó a los tokens, al catálogo y al encabezado. **Rige sobre la tienda, no sobre el panel**: la pantalla de panel del canvas no se adopta (§1.3), y del rediseño el panel solo hereda la capa de tokens, que es compartida por decisión de §4. **Las que faltan no son tareas nuevas de rediseño:** F3.7 (home) y F5.5 (carrito) se construyen ya sobre el lineamiento —F3.5 se construyó así el 2026-09-08— y el pie de página entra con la pantalla que lo obligue a cambiar. Rediseñar por separado algo que todavía no existe es hacer dos veces el mismo trabajo.
...
| **F3.4** | Catálogo: filtros, orden, paginación, todo en la URL | **L** | FS RF-02 · TS §10.2 · DR §7.2 | Categoría, marca y color **multiselección** —se suman dentro del grupo, se cruzan entre grupos— más **rango de precio** sobre el precio final. El botón atrás funciona; la URL es compartible. **Un chip removible por valor aplicado** y «Limpiar todo» |
| **F3.5** | Ficha de producto con galería y selector de color | **L** | FS RF-03, RF-10, RN-04c, RN-10 · DR §6.5, §6.8, §7.3 | Cambiar de color cambia imágenes y stock sin recargar, y la URL lo refleja. Producto inactivo devuelve 404. **La variante sin stock lo dice con palabras** y su botón principal pasa a ser «Preguntá si va a haber» (F3.6), con la aclaración de que nadie va a avisar automáticamente. **Galería con miniaturas a la izquierda, siempre dibujadas**, flechas, foto al borde de la tarjeta y visor de dos niveles —encuadrada y a tamaño real, recorrible— sin botón de ampliar. **La galería queda pegada y la columna derecha desplaza**, con los datos, la descripción y el recuadro «¿Cómo sigue después de comprar?» adentro. **Guardar y Compartir** debajo de las acciones: el corazón ya no va sobre la imagen en la ficha |
...
| **F3.7** | Home | M | FS RF-01 · DR §7.1 | **Hero con bloques de producto que rotan de categoría y se pueden parar** (WCAG 2.2.2), buscador, chips de categoría, una sección por categoría destacada, destacados, ofertas, medios de pago y el aviso de zona (RN-10). *Corregido el 2026-09-22 con tu boceto: el aviso subió al hero y ahí va **en una línea**; la versión larga, con el retiro en el punto de entrega, la sigue diciendo el pie del sitio, que está en todas las pantallas* |
...
| **F5.3** | Libreta de direcciones | M | FS RF-09 · TS §5.5 | **Hasta 3 direcciones** (2026-09-13), una predeterminada garantizada por la base. Estrena el menú de secciones de «Mi cuenta» |
| **F5.4** | Favoritos | S | FS RF-10 · DR §6.1, §7.3 | Desde tarjeta y ficha, **dibujado distinto en cada una y a propósito** (§7.3): en la tarjeta es el corazón solo, arriba a la derecha de la imagen; en la ficha es el botón **«Guardar»** que ya está en pantalla, hoy deshabilitado. En las dos: contorno sin marcar, relleno `--brand` marcado, y el estado anunciado además del relleno. Llena el hueco `accionFavorito` de la tarjeta (F3.1) y enciende el botón de la ficha (F3.5). Un producto desactivado aparece como «No disponible» |
| **F5.5** | Carrito: modelo y operaciones | M | FS RF-08 · TS §5.5 | Agregar, quitar, cambiar cantidad, vaciar. **No existe carrito sin sesión**. Enciende el botón «Agregá al carrito» que F3.5 dejó dibujado y apagado, y se lleva con él su explicación para lectores de pantalla |
...
| **F7.2a** | **Sumar a una orden activa: subir cantidades y agregar productos** | M | FS RF-22 (sobre F7.2 y F7.4) | Se sube la cantidad de un renglón y se agrega un producto que no estaba, reservando el stock con el `UPDATE` condicional de TS §8.2. **Sin stock no se agrega y se dice cuánto hay**, al revés de RF-24, que advierte sin bloquear porque ahí la venta ya ocurrió. Las unidades que se suman a un renglón van al precio de ese renglón; un producto nuevo entra al precio vigente. **Va después de F7.4 y no al lado de F7.2**: necesita el mismo buscador de variantes con cantidad que F7.4 construye, y hacerlo antes sería escribirlo dos veces |
...
> **F7.2 y F7.2a son las dos mitades de RF-22 y están separadas a propósito.** Quitar siempre se puede: es soltar una reserva y nada puede fallar, así que F7.2 se apoya entera en lo que F4.4 ya dejó probado. Sumar puede no poderse —hay que reservar, y el stock puede no estar—, y además necesita elegir un producto, que es la pieza grande de F7.4. Juntarlas habría hecho una tarea con dos mitades de dificultad muy distinta, y habría dejado la mitad fácil esperando a la difícil. **Hueco detectado el 2026-09-16**, revisando F7.2: RF-22 estaba escrito sólo para restar.
