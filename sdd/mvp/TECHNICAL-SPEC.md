...
CREATE TABLE orders (
  ...
  user_id           uuid REFERENCES user_profiles(id) ON DELETE RESTRICT, -- NULL en órdenes manuales sin cuenta
...
> **`user_id` es `RESTRICT`, y antes decía `SET NULL`.** Corregido en F4.5b (migración `0010`) por una contradicción que estaba escrita acá mismo, dos líneas más abajo: `SET NULL` dice «la orden sobrevive a que se borre la cuenta» y el CHECK `web_order_has_user` dice «una orden web siempre tiene comprador». Las dos no pueden ser ciertas. Al borrar una identidad, la cascada intentaba poner el `user_id` en NULL y el CHECK la rechazaba: el borrado fallaba igual, pero con una violación de CHECK que no explica nada. Apareció cuando la limpieza de un test de F4.3 reventó.
>
> **Se resolvió del lado de conservar**, por decisión del 2026-09-06: los usuarios no se eliminan. No es una regla nueva — **RF-26 nunca tuvo «eliminar usuario»** (lista, crea, modifica, resetea contraseña y bloquea) y RF-07 no tiene «cerrar mi cuenta». `SET NULL` estaba escrito como si borrar fuera una función que existe. Con `RESTRICT` la base dice lo mismo que los requisitos, y cuando alguien lo intente desde Studio o la Admin API va a fallar **nombrando el motivo**: esta persona tiene órdenes. Es además el patrón que el esquema ya usa para no destruir historia (`returns.order_id`, `return_items.order_item_id`).
>
> **Un usuario sin órdenes sí se puede borrar**, y tiene que poder: la compensación de §13.4 paso 3 hace exactamente eso cuando el alta del perfil falla. Ahí no hay nada que conservar.
>
> **`created_by` sigue en `SET NULL`** a propósito: es la administradora que cargó una orden manual (RF-24), no su comprador, y ninguna restricción exige que esté. Perder ese dato empobrece la auditoría pero no deja la fila inconsistente. Lo mismo vale para `actor_user_id` en el historial y en el libro mayor.
