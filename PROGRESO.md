...
| **El umbral se lee con respaldo hasta F2.7** | `TECHNICAL-SPEC.md` §10.3; `modules/settings/queries.ts` | `site_settings` es una fila única que todavía **no existe**: la escribe F2.7. Fallar sería dejar el listado caído por una configuración que nadie cargó aún, así que se usa el mismo `3` que declara la columna. La constante está repetida a propósito y no derivada: la base decide el valor de la fila nueva y el código decide qué pasa cuando no hay fila |
