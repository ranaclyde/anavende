...
| F7.2a | Sumar a una orden activa | ⬜ | **La otra mitad de RF-22, que el requisito no tenía.** Subir la cantidad de un renglón y agregar un producto que no estaba. **El hueco lo viste vos el 2026-09-16**, mirando F7.2: el requisito se escribió pensando en una sola situación —«me quedé sin uno de los tres»— y por eso sólo hablaba de quitar y reducir, pero la coordinación por WhatsApp va en los dos sentidos. Hoy la única salida sería cancelar la orden y rehacerla, o cargar una orden manual por el agregado, que parte una venta en dos y le arruina el historial al comprador. **Es más difícil que restar, y por eso es una tarea aparte**: quitar suelta una reserva y no puede fallar; sumar tiene que reservar, y el stock puede no estar. **Sin stock no se agrega**, al revés de la orden manual de RF-24, que advierte sin bloquear — ahí la venta ya ocurrió y acá todavía no, y reservar de más rompería el invariante que evita vender dos veces la misma unidad (§8.1). **Va después de F7.4** porque necesita el mismo buscador de variantes con cantidad que esa tarea construye |
